...
void CarB()
{
	...
	matrixPhanCapCarB = matrixPhanCapCarB * Translate(20, 1, 0);
...
void CarC_BanhSau()
{
	CarC_vl = RGBtoColor(30, 20, 30);
	CarC_kt = RGBtoColor(35, 30, 40);
	CarC_pxg = RGBtoColor(40, 40, 50);
	TaoVatLieu(CarC_vl, CarC_kt, CarC_pxg, 1000);

	// Bánh trái sau
	CarC_symbol = Translate(0, 0, -1.4) * Scale(1, 1, 0.3) * RotateX(90);
	VeHinhTru(CarC_symbol, matrixPhanCapCarC);
	...
	//Trục bánh xe
	CarC_vl = RGBtoColor(82, 135, 190);
	CarC_kt = RGBtoColor(75, 120, 180);
	CarC_pxg = RGBtoColor(70, 110, 170);
	TaoVatLieu(CarC_vl, CarC_kt, CarC_pxg, 1000);
	CarC_symbol = Scale(0.3, 0.3, 3.2) * RotateX(90);
	VeHinhTru(CarC_symbol, matrixPhanCapCarC);
}
...
void CarC_BanhTruoc()
{
	...
	CarC_symbol = Scale(1, 1, 0.3) * RotateX(90);
	VeHinhTru(CarC_symbol, matrixPhanCapCarC);
	...
	CarC_pxg = RGBtoColor(70, 110, 170);
	TaoVatLieu(CarC_vl, CarC_kt, CarC_pxg, 1000);
	CarC_symbol = Scale(0.3, 0.3, 0.36) * RotateX(90);
	VeHinhTru(CarC_symbol, matrixPhanCapCarC);
}

void CarC_ThanXe()
{
	CarC_vl = RGBtoColor(10, 100, 220);
	CarC_kt = RGBtoColor(30, 120, 200);
	CarC_pxg = RGBtoColor(50, 130, 180);
	TaoVatLieu(CarC_vl, CarC_kt, CarC_pxg, 1000);

	//Mặt dưới đầu xe
	CarC_symbol = Translate(3.05, -0.95, 0) * Scale(2.1, 0.1, 3);
	VeHinhLapPhuong(CarC_symbol, matrixPhanCapCarC);

	//Mặt sau đầu xe
	CarC_symbol = Translate(2, 0, 0) * Scale(0.1, 2, 3);
	VeHinhLapPhuong(CarC_symbol, matrixPhanCapCarC);

	//Mặt trước đầu xe
	CarC_symbol = Translate(3.95, -0.4, 0) * Scale(0.1, 1.1, 3);
	VeHinhLapPhuong(CarC_symbol, matrixPhanCapCarC);

	//Mặt trên đầu xe
	CarC_symbol = Translate(2.55, 0.95, 0) * Scale(1.2, 0.1, 3);
	VeHinhLapPhuong(CarC_symbol, matrixPhanCapCarC);

	//Thanh chéo trước đầu xe bên trái
	CarC_symbol = Translate(3.53, 0.54, -1.4) * RotateZ(-45) * Scale(1.2, 0.1, 0.2);
	VeHinhLapPhuong(CarC_symbol, matrixPhanCapCarC);
	...
	CarC_vl = RGBtoColor(200, 80, 80);
	CarC_kt = RGBtoColor(190, 90, 100);
	CarC_pxg = RGBtoColor(180, 100, 120);
	TaoVatLieu(CarC_vl, CarC_kt, CarC_pxg, 1000);

	// Thanh dưới thùng xe bên trái
	CarC_symbol = Translate(-1, -0.95, -1.25) * Scale(6, 0.1, 0.5);
	VeHinhLapPhuong(CarC_symbol, matrixPhanCapCarC);

	//Thanh dưới thùng xe ở giữa
	CarC_symbol = Translate(-1, -0.95, 0) * Scale(6, 0.1, 0.5);
	VeHinhLapPhuong(CarC_symbol, matrixPhanCapCarC);
	...
	//Thanh dưới thùng xe phía sau
	CarC_symbol = Translate(-3.75, -0.95, 0) * Scale(0.5, 0.1, 3);
	VeHinhLapPhuong(CarC_symbol, matrixPhanCapCarC);

	//Thanh dưới thùng xe phía trước
	CarC_symbol = Translate(1.5, -0.95, 0) * Scale(1, 0.1, 3);
	VeHinhLapPhuong(CarC_symbol, matrixPhanCapCarC);

	//Trang trí đầu xe
	CarC_vl = RGBtoColor(175, 100, 60);
	CarC_kt = RGBtoColor(160, 120, 80);
	CarC_pxg = RGBtoColor(150, 140, 100);
	TaoVatLieu(CarC_vl, CarC_kt, CarC_pxg, 1000);
	// Nóc xe
	for (size_t i = 0; i < 3; i++)
	{
		CarC_symbol = Translate((float)i / 3 + 2.2, 0.95, 0) * Scale(0.1, 0.14, 2.8);
		VeHinhLapPhuong(CarC_symbol, matrixPhanCapCarC);
	}

	// Hình trụ đầu xe
	CarC_symbol = Translate(4.03, -0.93, 0) * RotateX(90) * Scale(0.2, 3.02, 0.2);
	VeHinhTru(CarC_symbol, matrixPhanCapCarC);

	// Đèn xe
	CarC_vl = RGBtoColor(CarA_DenXe[0], CarA_DenXe[1], CarA_DenXe[2]);
	CarC_kt = RGBtoColor(CarA_DenXe[0] + 20, CarA_DenXe[1] + 20, CarA_DenXe[2] + 20);
	CarC_pxg = RGBtoColor(CarA_DenXe[0] + 40, CarA_DenXe[1] + 40, CarA_DenXe[2] + 40);
	TaoVatLieu(CarC_vl, CarC_kt, CarC_pxg, 1000);

	CarC_symbol = Translate(4, -0.5, 1) * RotateZ(90) * Scale(0.4, 0.1, 0.4);
	VeHinhTru(CarC_symbol, matrixPhanCapCarC);
	...
	//Ghế
	CarC_vl = RGBtoColor(129, 156, 96);
	CarC_kt = RGBtoColor(129, 156, 96);
	CarC_pxg = RGBtoColor(129, 156, 96);
	TaoVatLieu(CarC_vl, CarC_kt, CarC_pxg, 1000);
	CarC_symbol = Translate(2.6, -0.8, 0) * Scale(0.8, 0.3, 1.2);// ghế
	VeHinhLapPhuong(CarC_symbol, matrixPhanCapCarC);
	CarC_symbol = Translate(2.25, -0.15, 0) * Scale(0.1, 1.2, 1.2) * RotateZ(0);// tựa
	VeHinhLapPhuong(CarC_symbol, matrixPhanCapCarC);
}
...
void CarC_VoLang()
{
	...
	// Thanh Dài
	CarC_symbol = Translate(0, 0.5, 0) * Scale(0.1, 1, 0.1);
	VeHinhTru(CarC_symbol, matrixPhanCapCarC);

	//Bánh Xoay Vô Lăng
	CarC_symbol = Translate(0, 1, 0) * Scale(0.5, 0.1, 0.5);
	VeHinhTru(CarC_symbol, matrixPhanCapCarC);
}
...
{
	//Cánh cửa
	CarC_vl = RGBtoColor(175, 100, 0);
	CarC_kt = RGBtoColor(160, 120, 20);
	CarC_pxg = RGBtoColor(150, 140, 40);
	TaoVatLieu(CarC_vl, CarC_kt, CarC_pxg, 1000);

	CarC_symbol = Translate(-0.925, 0, 0) * Scale(1.85, 1, 0.1);
	VeHinhLapPhuong(CarC_symbol, matrixPhanCapCarC);

	//Tay nắm cửa
	CarC_vl = RGBtoColor(120, 120, 100);
	CarC_kt = RGBtoColor(140, 122, 120);
	CarC_pxg = RGBtoColor(150, 140, 140);
	TaoVatLieu(CarC_vl, CarC_kt, CarC_pxg, 1000);

	CarC_symbol = Translate(-1.5, 0, -0.05) * Scale(0.1, 0.3, 0.3);
	VeHinhTru(CarC_symbol, matrixPhanCapCarC);
}
...
void CarC_ThungXe()
{
	CarC_vl = RGBtoColor(150, 130, 30);
	CarC_kt = RGBtoColor(140, 120, 20);
	CarC_pxg = RGBtoColor(130, 110, 10);
	TaoVatLieu(CarC_vl, CarC_kt, CarC_pxg, 1000);

	//Mặt dưới
	CarC_symbol = Translate(2.75, 0.1, 0) * Scale(5.6, 0.2, 3.6);
	VeHinhLapPhuong(CarC_symbol, matrixPhanCapCarC);

	//Mặt trái
	CarC_symbol = Translate(2.75, 1, 1.65) * Scale(5.5, 2, 0.1);
	VeHinhLapPhuong(CarC_symbol, matrixPhanCapCarC);
	...
	//Mặt trước
	CarC_symbol = Translate(5.5, 1, 0) * Scale(0.1, 2, 3.6);
	VeHinhLapPhuong(CarC_symbol, matrixPhanCapCarC);

	CarC_symbol = Translate(2.75, 1.9, 1.7) * Scale(5.5, 0.2, 0.2);
	VeHinhLapPhuong(CarC_symbol, matrixPhanCapCarC);

	CarC_symbol = Translate(0, 1, 1.7) * Scale(0.1, 2, 0.2);
	VeHinhLapPhuong(CarC_symbol, matrixPhanCapCarC);
	...
	for (int i = 1; i <= 11; i++)
	{
		CarC_symbol = Translate((float)i / 2, 1, 1.7) * Scale(0.1, 2, 0.1);
		VeHinhLapPhuong(CarC_symbol, matrixPhanCapCarC);
	}
	...
	CarC_vl = RGBtoColor(192, 98, 58);
	CarC_kt = RGBtoColor(192, 98, 58);
	CarC_pxg = RGBtoColor(192, 98, 58);
	TaoVatLieu(CarC_vl, CarC_kt, CarC_pxg, 1000);

}

void CarC_CuaThungXe()
{
	CarC_vl = RGBtoColor(150, 110, 10);
	CarC_kt = RGBtoColor(130, 110, 20);
	CarC_pxg = RGBtoColor(130, 110, 30);
	TaoVatLieu(CarC_vl, CarC_kt, CarC_pxg, 1000);

	CarC_symbol = Translate(0, -0.9, 0) * Scale(0.1, 1.8, 3.2);
	VeHinhLapPhuong(CarC_symbol, matrixPhanCapCarC);

	CarC_vl = RGBtoColor(115, 10, 10);
	CarC_kt = RGBtoColor(130, 30, 20);
	CarC_pxg = RGBtoColor(140, 40, 30);
	TaoVatLieu(CarC_vl, CarC_kt, CarC_pxg, 1000);

	CarC_symbol = Translate(0, -0.05, 0) * RotateX(90) * Scale(0.05, 3.65, 0.05);
	VeHinhTru(CarC_symbol, matrixPhanCapCarC);

	for (size_t i = 0; i < 4; i++)
	{
		CarC_symbol = Translate(-0.05, (float)i / 3 - 1.5, 0) * Scale(0.05, 0.05, 3);
		VeHinhLapPhuong(CarC_symbol, matrixPhanCapCarC);
	}

}

void CarC_BenNangThung()
{
	CarC_vl = RGBtoColor(195, 90, 50);
	CarC_kt = RGBtoColor(200, 100, 65);
	CarC_pxg = RGBtoColor(205, 110, 70);
	TaoVatLieu(CarC_vl, CarC_kt, CarC_pxg, 1000);

	CarC_symbol = Translate(0, 0.5, 0) * Scale(0.3, 1, 0.3);
	VeHinhTru(CarC_symbol, matrixPhanCapCarC);
}

void CarC()
{
	matrixPhanCapCarC = mat4();
	matrixPhanCapCarC = matrixPhanCapCarC * Translate(55, 0, -45);
	matrixPhanCapCarC = matrixPhanCapCarC * Translate(0, 1, 0) * RotateY(0);
	CarC_ThanXe();
	mat4 _MatrixTam = matrixPhanCapCarC;
	matrixPhanCapCarC = matrixPhanCapCarC * Translate(-4, -0.85, 0) * RotateZ(0);
	CarC_ThungXe();
	matrixPhanCapCarC = matrixPhanCapCarC * Translate(0, 2, 0) ;
	CarC_CuaThungXe();
	matrixPhanCapCarC = _MatrixTam * Translate(-2, -1.5, 0);
	CarC_BanhSau();
	matrixPhanCapCarC = _MatrixTam * Translate(3.9, -0.4, -1.45);
	CarC_CuaXeTrai();
	matrixPhanCapCarC = _MatrixTam * Translate(3.9, -0.4, 1.45) ;
	CarC_CuaXePhai();
	matrixPhanCapCarC = _MatrixTam * Translate(2.5, -1.5, -1.4);
	CarC_BanhTruoc();
	matrixPhanCapCarC = _MatrixTam * Translate(2.5, -1.5, 1.4);
	CarC_BanhTruoc();
	matrixPhanCapCarC = _MatrixTam * Translate(-0.3, -1, 0) * RotateZ(30) * Scale(1, 4.1 * sinf(DEGtoRAD(0)), 1);
	CarC_BenNangThung();
	matrixPhanCapCarC = _MatrixTam * Translate(3.9, -0.7, 0) * RotateZ(30) * RotateY(0 * 2);
	CarC_VoLang();
...
{
	matrixPhanCapCarC = mat4();
	matrixPhanCapCarC = matrixPhanCapCarC * Translate(55, 0, -25);
	matrixPhanCapCarC = matrixPhanCapCarC * Translate(0, 1, 0) * RotateY(0);
	CarC_ThanXe();
	mat4 _MatrixTam = matrixPhanCapCarC;
	matrixPhanCapCarC = matrixPhanCapCarC * Translate(-4, -0.85, 0) * RotateZ(0);
...
#pragma endregion CarC


#pragma region Bot Thu Phi
// Biến matrix dùng cho mô hình phân cấp
mat4 matrixPhanCapBot;
color4 Bot_vl;
color4 Bot_kt;
color4 Bot_pxg;
mat4 Bot_symbol;
...
void CanNangIn()
{
	Bot_vl = RGBtoColor(130, 130, 65);
	Bot_kt = RGBtoColor(130, 130, 65);
	Bot_pxg = RGBtoColor(130, 130, 65);
	TaoVatLieu(Bot_vl, Bot_kt, Bot_pxg, 100);

	Bot_symbol = RotateZ(90) * RotateY(-canNangIn) * Scale(0.4, 2, 0.4);
	VeHinhTru(Bot_symbol, matrixPhanCapBot);

	Bot_symbol = Translate(0.5, 0, 0) * RotateX(canNangIn) * Translate(0, 0, -5.8) * Scale(0.2, 0.6, 13.6);
	VeHinhLapPhuong(Bot_symbol, matrixPhanCapBot);
}
...
void CanNangOut()
{
	...
	Bot_symbol = RotateZ(90) * RotateY(canNangOut) * Scale(0.4, 2, 0.4);
	VeHinhTru(Bot_symbol, matrixPhanCapBot);

	Bot_symbol = Translate(-0.5, 0, 0) * RotateX(canNangOut) * Translate(0, 0, 5.8) * Scale(0.2, 0.6, 13.6);
	VeHinhLapPhuong(Bot_symbol, matrixPhanCapBot);
}
...
void Bot()
{
	...
	TaoVatLieu(Bot_vl, Bot_kt, Bot_pxg, 100);
	//Dat
	Bot_symbol = Scale(12, 0.1, 12);
	VeHinhLapPhuong(Bot_symbol, matrixPhanCapBot);

	// Dinh
	Bot_symbol = Translate(0, 8, 0) * Scale(15, 0.1, 14);
	VeHinhLapPhuong(Bot_symbol, matrixPhanCapBot);
	Bot_vl = RGBtoColor(130, 130, 165);
	Bot_kt = RGBtoColor(130, 130, 165);
	Bot_pxg = RGBtoColor(130, 130, 165);
	TaoVatLieu(Bot_vl, Bot_kt, Bot_pxg, 100);
	Bot_symbol = Translate(0, 8.3, 0) * Scale(10, 0.5, 9);
	VeHinhLapPhuong(Bot_symbol, matrixPhanCapBot);
	...
	Bot_pxg = RGBtoColor(130, 130, 65);
	TaoVatLieu(Bot_vl, Bot_kt, Bot_pxg, 100);
	/*Bot_symbol = Translate(0, 4, 4.5) * Scale(10, 8, 0.1);
	VeHinhLapPhuong(Bot_symbol, matrixPhanCapBot);*/
	//Tuong sau
	Bot_symbol = Translate(4.9, 4, 0) * Scale(0.1, 8, 9);
	VeHinhLapPhuong(Bot_symbol, matrixPhanCapBot);

	//Tuong truoc
	Bot_symbol = Translate(-4.9, 4, 2.5) * Scale(0.1, 8, 4);
	VeHinhLapPhuong(Bot_symbol, matrixPhanCapBot);
	Bot_symbol = Translate(-4.9, 4, -3.5) * Scale(0.1, 8, 2);
	VeHinhLapPhuong(Bot_symbol, matrixPhanCapBot);

	Bot_symbol = Translate(-4.9, 6.5, -1) * Scale(0.1, 3, 3);
	VeHinhLapPhuong(Bot_symbol, matrixPhanCapBot);

	// Tuong ben trai
	Bot_vl = RGBtoColor(230, 130, 165);
	Bot_kt = RGBtoColor(230, 130, 165);
	Bot_pxg = RGBtoColor(230, 130, 165);
	TaoVatLieu(Bot_vl, Bot_kt, Bot_pxg, 100);
	Bot_symbol = Translate(0, 7, -4.5) * Scale(10, 2, 0.1);
	VeHinhLapPhuong(Bot_symbol, matrixPhanCapBot);

	Bot_symbol = Translate(0, 1.5, -4.5) * Scale(4, 3, 0.1);
	VeHinhLapPhuong(Bot_symbol, matrixPhanCapBot);

	//Thanh doc
	Bot_symbol = Translate(-3.5, 3, -4.5) * Scale(3, 6, 0.1);
	VeHinhLapPhuong(Bot_symbol, matrixPhanCapBot);
	Bot_symbol = Translate(3.5, 3, -4.5) * Scale(3, 6, 0.1);
	VeHinhLapPhuong(Bot_symbol, matrixPhanCapBot);
	...
	//Can nang IN
	Bot_symbol = Translate(0, 0, 20) * Scale(10, 1, 4);
	VeHinhLapPhuong(Bot_symbol, matrixPhanCapBot);
	Bot_symbol = Translate(3.6, 2, 20) * Scale(0.4, 4, 1);
	VeHinhLapPhuong(Bot_symbol, matrixPhanCapBot);
...
void Hang_Rao_Full()
{
	Bot_vl = RGBtoColor(170, 170, 85);
	Bot_kt = RGBtoColor(180, 180, 90);
	Bot_pxg = RGBtoColor(190, 190, 95);
	TaoVatLieu(Bot_vl, Bot_kt, Bot_pxg, 100);

	//Hai thanh ngang
	Bot_symbol = Scale(0.1, 0.1, 80);
	VeHinhLapPhuong(Bot_symbol, matrixPhanCapBot);
	Bot_symbol = Translate(0, 1.5, 0) * Scale(0.1, 0.1, 80);
	VeHinhLapPhuong(Bot_symbol, matrixPhanCapBot);

	//Các thanh dọc
	for (int i = -40; i < 41; i++)
	{
		Bot_symbol = Translate(0, 0.5, i) * Scale(0.1, 3, 0.1);
		VeHinhLapPhuong(Bot_symbol, matrixPhanCapBot);
	}
}

float moCuaDiBot = 1.5;
void CanhCuaDiBot()
{
	//Canh cua di
	Bot_vl = RGBtoColor(230, 30, 165);
	Bot_kt = RGBtoColor(230, 30, 165);
	Bot_pxg = RGBtoColor(230, 30, 165);
	TaoVatLieu(Bot_vl, Bot_kt, Bot_pxg, 100);
	Bot_symbol = Scale(0.1, 5, 3);
	VeHinhLapPhuong(Bot_symbol, matrixPhanCapBot);
	...
	Bot_pxg = RGBtoColor(230, 130, 165);
	TaoVatLieu(Bot_vl, Bot_kt, Bot_pxg, 100);
	Bot_symbol = Translate(0, 0, -1.3) * Scale(0.3, 0.6, 0.1);
	VeHinhLapPhuong(Bot_symbol, matrixPhanCapBot);
}

void BotThuPhi()
{
	matrixPhanCapBot = mat4();
	/*matrixPhanCapBot = matrixPhanCapBot * Translate(-50, -1, 30);*/
	matrixPhanCapBot = matrixPhanCapBot * Translate(-60, -1, 0);
	mat4 _matrix = matrixPhanCapBot;
	Bot();
	matrixPhanCapBot = _matrix * Translate(-4.8, 2.5, -1 + moCuaDiBot);
	CanhCuaDiBot();
	matrixPhanCapBot = _matrix * Translate(4, 3.5, 20.05);
	CanNangIn();
	matrixPhanCapBot = _matrix * Translate(-4, 3.5, -20.05);
	CanNangOut();
	matrixPhanCapBot = _matrix * Translate(-5, 1, -62);
	Hang_Rao_Full();
	matrixPhanCapBot = _matrix * Translate(-5, 1, 62);
	Hang_Rao_Full();
}
...
void Cai_San()
{
	...
	// Vẽ cái cây
	CaiCayTo(Translate(-39, 0.1, -33) * RotateY(145));
	CaiCayTo(Translate(-38, 0.2, 36) * RotateY(75) * Scale(0.7, 0.7, 0.7));
...
void IdleControl()
{
	...
	{
		RungLaCay += 0.02;
		if (RungLaCay >= 2) RungLaCayBenPhai = !RungLaCayBenPhai;
	}
	else
	{
		RungLaCay -= 0.02;
